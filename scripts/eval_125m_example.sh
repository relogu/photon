...
if ! OPTIONS=$(getopt -o p: --long project_path: -n 'parse-options' -- "$@"); then
	echo "eval_125m_example.sh: Error parsing options" >&2
	exit 1
fi
...
echo "eval_125m_example.sh: PROJECT_PATH=$PROJECT_PATH"
...
#! Additional configs
EXTERNAL_CONFIGS="$EXTERNAL_CONFIGS llm_config.device_eval_batch_size=1"
...
EXTERNAL_CONFIGS="$EXTERNAL_CONFIGS llm_config.eval_first=true"
...
bash $PROJECT_PATH/scripts/eval_gauntlet_only.sh -p "$PROJECT_PATH" 125M
